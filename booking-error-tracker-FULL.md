# Booking.com Error Tracker - FULL UNOBFUSCATED CODE

> Original file: error---c57ab48f-82f2-4d80-a329-d6676e10daf6.txt
> Date: June 28, 2026

---

## PART 1: MAIN IIFE (Immediately Invoked Function Expression)

```javascript
/**
 * Booking.com Error Tracker & Bot Detection
 * 
 * This code runs immediately when loaded. It wraps:
 * 1. JavaScript error handling (window.onerror)
 * 2. Bot detection (checks specific window keys)
 * 3. Third-party script error tracking
 * 4. jQuery event wrapping (tracks clicks)
 * 5. setTimeout/setInterval wrapping
 * 6. Server-side error reporting
 * 
 * Arguments:
 * - win: window object
 * - doc: document object
...
// Array to store all collected errors
var errors = [],
    errorCount = 0,

    // Can we parse functions? (feature detection)
    // Test: Can we use regex on function.toString()?
    canParse = (function() {}).toString && /bkg/.test( function() { bkg; } );

// Current timestamp (used for timing calculations)
var NOW,
    UNDEFINED_VALUE;

// Track the LAST element the user clicked on
// Used to identify user behavior patterns
// Format: "click on #password" or "click on input.username"
var LAST_CLIENT_EVENT;

// ============================================================
// SECTION 2: SERVER BLOCKING MECHANISM
// ============================================================

/**
 * The server can tell us to stop reporting errors.
 * This happens if we send too many errors or get flagged.
 * 
 * How it works:
 * 1. Server sets a cookie: "error_catcher=kill"
 * 2. We read that cookie on page load
 * 3. If cookie is "kill", we stop sending errors
 * 
 * Cookie lasts 30 days once set
 */
var SERVER_ASKED_TO_BLOCK = readCookie( 'error_catcher' ) === 'kill';

/**
 * Should we block this error?
 * 
 * Conditions to block:
...
// ============================================================
// SECTION 3: ERROR TRANSPORT CONFIGURATION
// ============================================================
...
// Define all error collection functions
var ERROR_DATA_COLLECTION = {
    ...
    lno: function( message, url, line, column, error ) {
        return Number( line ) ? line : this.getFileOffsetFromError( error ).line || UNDEF;
    },
    ...
    cno: function( message, url, line, column, error ) {
        return Number( column ) ? column : this.getFileOffsetFromError( error ).column || UNDEF;
    },
    ...
    /**
     * Get SITE TYPE ID (b_site_type_id)
    ...
    /**
     * Get LANGUAGE for URL (b_lang_for_url)
 ...
 * 
 * @param items - Space-separated list
 * @param callback - Function to call for each
 ...
 * @param handlerCallback - Called with arguments before
 * @param resultCallback - Called with result after
 */
 ...
 * 
 * Checks:
 * 1. Error source URL (source mapping)
 * 2. Stack trace (stack trace mapping)
 * 3. Error message (message mapping)
 * 4. Template errors (jstmpl:: prefix)
 * 
 * Returns: { is: true/false, key: 'name' }
 */
function thirdPartyCheck( message, stack, url ) {
    ...
    // Check for external source (not booking.com or bstatic.com)
    // And not a template error (jstmpl::)
 ...
 * 
 * Limits:
 * - Max lines: MAX_STACK_LINES (12)
 * - Max chars: MAX_STACK_LENGTH (900)
 */
 ...
 * 
 * Builds full stack from:
 * 1. Error stack (primary)
 * 2. Function caller chain (generated)
 */
function getErrorStackTrace( error, functionCaller, message ) {
    ...
    // If we have a function caller, build generated stack
    if ( functionCaller && message ) {
        ...
        while ( functionCaller ) {
            ...
            // Get file from error or current location
            ...
            // Add to lines: "functionName@(file:line:column)"
        ...
        // If we have lines, build full stack
        if ( lines.length ) {
 ...
 * 
 * Error groups identify where errors occur
 * Format: "FULL_STACK_filename->functionName():message"
 */
function getErrorGroupObj(stack, caller) {
    ...
    var location           = win.document.location || {},
        ...
        // Extract filename from stack: "//domain/path:line:column"
        fileName = ( stack.match( /\/\/[^/]+(\/[^: ]+):(\d+):(\d+)/ ) || [ '', '' ] )[ 1 ].split( '?' )[ 0 ] || ( host + path ),
        
        // Extract function name from stack
        functionName = callerName || 
                     ( stack.match( /(?:at)?\s+([0-9a-zA-Z_]+)[\s@]+(?:\(?https?:)?\/\/[^/]+\/[^: ]+:\d+:\d+/ ) || [] )[ 1 ],
        
        // Extract function source
        functionSource = callerSource || 
                       ( stack.match( /(function\([^\)]*\)\{[^@]+\})@(?:\(?https?:)?\/\/[^/]+\/[^: ]+:\d+:\d+/i ) || [] )[ 1 ],
    ...
    // Clean up versioned filenames: ".v123v.js" -> ".js"
    fileName.replace( /(\.v[a-zA-Z0-9]+v\.)js$/, '.js' );

    // Use custom group name if set
 ...
 * 1. B0T - Bot detected
 * 2. THIRD_PARTY - External script error
 * 3. INTERNAL - Booking's own error
 */
function getErrorObj( message, url, line, column, error, caller ) {
     ...
     * 1. B0T - Bot detected (IS_BOT returns true)
     * 2. THIRD_PARTY - From external script
     * 3. INTERNAL - From Booking's own code
     */
 ...
 * 
 * Example: getKey('booking_extra.pageview_id')
 * Returns: window.booking_extra.pageview_id
 * 
 * Supports paths with dots AND spaces
 */
 ...
 * Only overwrites keys that already exist
 */
 ...
 * 
 * Uses XMLHttpRequest (modern) or ActiveXObject (old IE)
 */
function beacon( config, onResponse ) {
    ...
    // GET for GET requests, append data to URL
...
function getFileOffsetFromError( error ) {
    ...
    // Parse from stack or error number
...
function getFunctionCaller( args ) {
    ...
    try {
        ...
        // Try to get caller through callee
...
// ============================================================
// END PART 1: MAIN IIFE
// ============================================================



// ============================================================
// PART 2: PROTOCOL CHECK (AT END OF FILE)
// ============================================================
...
try {
  (function(){
      ...
      // Get protocol in lowercase (without colon)
      var t=window.location.protocol.toLowerCase().substr(0,window.location.protocol.length-1);
      
      // Get host in lowercase, remove trailing dot
      var n=window.location.host.toLowerCase().replace(/\.$/, '');
      
      // Check protocol is NOT https
      if(t!=="https")e=true;
      
      // Check host does NOT end with .booking.com
      if(n.substr(n.length-12)!==".booking.com")e=true;
      
      // If any check failed, report it
      if(e){
          // Build reporting URL
          var r="https://www.booking.com/_etnht"
              + "?cpr="+encodeURIComponent(t)        // protocol
              + "&ch="+encodeURIComponent(n)         // host
              + "&we=we"                         // flag
              + "&cpa="+encodeURIComponent(window.location.pathname);  // current path
              
          // Add application info if available
          if(window.$u!==undefined&&$u.application!==undefined){
              r+="&a="+encodeURIComponent($u.application.application);  // app name
              if($u.application.tag!==undefined)
                  r+="&at="+encodeURIComponent($u.application.tag); // app tag
          }
          
          // Add referrer if available
          if(document.referrer!==undefined&&document.referrer!="")
              r+="&cr="+encodeURIComponent(document.referrer);  // referrer
              
          // Send report via image beacon
 ...
 * 
 * Allows Booking's code to manually report errors
 */
 ...
 * @param title - Optional group name
 */
booking.reportError = function( err, title ) {

  var message = '', 
      prefix = '[' + ( title || 'Reported Error' ) + '] ';

  try {
    // Build message (max 500 chars)
    message = ( prefix + ( err.message || '' ) + ' ' + ( err.stack || '' ) ).slice( 0, 500 );
  } catch( e ) {}

  // If we have a message, report it
  if ( message ) {
    window.onerror( message );
  }

};
// Create window.booking if it doesn't exist
...
/*
ERROR_COLLECTION KEYS (from ERROR_DATA_COLLECTION):
--------------------------------------
