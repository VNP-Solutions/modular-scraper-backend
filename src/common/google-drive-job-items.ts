...
/**
 * `drive.file` is too narrow for shared-drive / shared-folder setups. Full `drive`
 * scope is used so the service account can write where it has been granted access.
 */
...
/** `{Portfolio}-{Property}-{DD-MM-YYYY}.xlsx` */
...
/**
 * root → Booking → DD-MM-YYYY → upsert `{Portfolio}-{Property}-{date}.xlsx`
 */
export async function uploadBookingJobItemsXlsxToGoogleDrive(
  xlsxBuffer: Buffer,
  ...
  const bookingFolderId = await findOrCreateChildFolder(
    drive,
    rootId,
    BOOKING_DRIVE_SUBFOLDER
  );
