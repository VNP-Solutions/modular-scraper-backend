 ...
 * calling updateJobStatus again, so the inner catch's reason is not overwritten.
 */
...
export function getFailedReasonForUser(
  ...
  // When OTP fails we close the browser; a follow-on cookies() call can throw
  // "Session closed". Map that to the same message as OTP timeout so the user
  // sees the real cause (verification step timed out), not the protocol error.
