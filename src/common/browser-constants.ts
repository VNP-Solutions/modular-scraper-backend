 ...
 * overwritten at runtime by `buildClientHintsFromUA()` so they always match
 * the actual bundled Chromium version. They are kept here only so that GraphQL
 * API calls made outside of a live browser context still have a sensible
 * fallback value.
 */
