...
app.get("/", (req, res, next) => {
  try {
    res.status(200).json({
      messge: "Connection established on expedia-retrive branch",
    });
  } catch (err: any) {
...
app.post("/api/scraping/resume", (async (
  ...
  try {
    ...
    if (normalizedOtaProvider === "Expedia") {
      ...
      return res.status(400).json({
        status: 400,
        message: `Unsupported OTA provider: ${normalizedOtaProvider}. Supported values are: 'Expedia'`,
      });
    }
...
app.post("/api/expedia/retrieval-run-job", (async (
  ...
  try {
    ...
    // Use retrieval_id directly as jobId since it's a valid MongoDB ObjectId
    // This ensures OTP operations work correctly without needing to extract ObjectId
